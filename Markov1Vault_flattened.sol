...
// File @openzeppelin/contracts/interfaces/IERC4626.sol@v5.6.1
...
// File @openzeppelin/contracts/utils/Memory.sol@v5.6.1
...
// File @openzeppelin/contracts/token/ERC20/extensions/ERC4626.sol@v5.6.1
...
// File contracts/Markov1Vault.sol
...
/**
 * @title Markov1Vault (ERC-4626 Non-Custodial Quant Vault)
 * @notice Accepts USDC deposits, issues mvUSDC shares, and authorizes the VPS Keeper
 *         to route trades through Hyperliquid L2 or Arbitrum Perps.
 */
contract Markov1Vault is ERC4626, Ownable, ReentrancyGuard {
    
    // --- ROLES & CONFIGURATION ---
    address public keeperNode;               // VPS Bot Keeper Address
    uint256 public performanceFeeBps = 1500; // 15.0% Performance Fee
    uint256 public constant BPS_DENOMINATOR = 10000;
    
    // --- ON-CHAIN RISK CONSTRAINTS ---
    uint256 public maxPositionRiskBps = 250; // Max 2.5% single-trade allocation
    uint256 public minWinProbThreshold = 5400; // 54.0% Monte Carlo Gate
    ...
    /**
     * @notice Authorizes the Python Keeper engine to trigger trade execution
     * @param symbol Trading pair (e.g., "BTC-PERP")
     * @param regime Market State ("Bullish", "Bearish", "Range-Bound")
     * @param winProb Probability scaled by 10000 (e.g., 5400 = 54.0%)
     */
    function executeQuantSignal(
        string calldata symbol,
        string calldata regime,
        uint256 winProb
    ) external onlyKeeper nonReentrant returns (uint256 tradeAllocation) {
        require(winProb >= minWinProbThreshold, "Markov1Vault: Win probability below 54.0% gate");

        // Calculate maximum allowed trade size (2.5% of total vault assets)
        ...
        // Signal is passed to Hyperliquid bridge or L2 adapter
